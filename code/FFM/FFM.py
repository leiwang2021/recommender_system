...
def createTwoDimensionWeight(input_x_size,field_size,vector_dimension):
    weights = tf.truncated_normal([input_x_size,field_size,vector_dimension])  #nfk
...
def createOneDimensionWeight(input_x_size):
    weights = tf.truncated_normal([input_x_size])   #n
    tf_weights = tf.Variable(weights)
    return tf_weights

def createZeroDimensionWeight():
    weights = tf.truncated_normal([1])   #1
    tf_weights = tf.Variable(weights)
    return tf_weights
...
def inference(input_x,input_x_field,zeroWeights,oneDimWeights,thirdWeight):
    ...
    for i in range(input_shape):
        ...
        for j in range(i+1,input_shape):
            ...
            weightLeftAfterCut = tf.squeeze(weightLeft)  #Vi,fj
            ...
            weightRightAfterCut = tf.squeeze(weightRight)  #Vj,fi
            ...
            product = tf.reduce_sum(tf.multiply(xi, xj))  #Xi*Xj
